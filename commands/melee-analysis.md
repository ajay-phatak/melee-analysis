...
7. **(Optional) Update Obsidian** via the `obsidian-mcp-connector` MCP server (vault-relative paths under `Melee Coach/`; skip this whole step if you don't use Obsidian — history + trends still work). Create the folder/notes if missing; for existing notes, read first and update in place (don't duplicate). Match your terse, bulleted note style.
   ...
   - **Obsidian linking convention (mandatory — apply on every note, every run).** These notes are a linked graph, not flat files. A link target is the note **basename only, no folder path** — Obsidian resolves it. Valid targets: any file in `Melee Coach/Matchups/` (e.g. `[[Sheik vs Marth]]`, `[[Fox vs Captain Falcon]]`), plus `[[Progress]]` and `[[user_profile]]`. **Before writing, list the `Melee Coach/Matchups/` folder and only link basenames that actually exist**; if the prose wording differs from the filename, use an alias link (`[[Sheik vs Captain Falcon|Sheik vs Falcon]]`). Never link a note that doesn't exist — create it first (every matchup played gets a note this run anyway).
     - **Session note** — (a) every per-matchup section heading wraps the matchup name: `## [[Sheik vs Marth]] — W 2-0 (OZY)`; (b) the trend section heading is `## vs your trend ([[Progress]])`; (c) the records line links each matchup: `- Records: [[Sheik vs Marth]] 12–4 · [[Sheik vs Falco]] 44–64.`; (d) the **last line of the note** is a trailer listing every matchup note the session touched, then Progress: `→ [[Sheik vs Marth]] · [[Sheik vs Fox]] · [[Progress]]`.
     - **Matchup note** — the blockquote intro line under the H1 always links back. Main-character notes: `> Neutral gameplan / flowchart. Tech execution lives in [[Progress]]. <extras>`. Secondary-character notes: `> Your **secondary (Fox), in development** — see [[user_profile]]. Neutral gameplan / flowchart; Sheik tech focuses live in [[Progress]]. <extras>`. Cross-reference a sibling matchup inline when a fix carries across them (e.g. "(Same fix logged in [[Fox vs Captain Falcon]].)").
     - **Progress.md** — every row of the per-matchup record table links its matchup name.
     - Density: link the headings, the records/trend lines, sibling cross-references, and the footer trailer. Do **not** link every inline repeat inside dense bullet prose — first mention per section is enough.
